    ...
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
      ...
      <div className={`relative z-10 text-center px-4 max-w-5xl mx-auto pt-24 transition-all duration-1000 ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
        ...
        <h1 className="font-display text-5xl md:text-7xl font-bold leading-tight mb-6">
          <span className="text-field">Preston Ridge</span><br />
          <em className="shimmer-text italic">Boykin Spaniels</em>
        </h1>

        <p className="font-rustic text-field/55 text-lg md:text-xl max-w-2xl mx-auto mb-4 font-light leading-relaxed italic">
          "The dog that doesn't rock the boat"
        </p>
        <p className="text-field/50 text-base max-w-2xl mx-auto mb-10 font-light leading-relaxed">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 max-w-3xl mx-auto">
          {badges.map(({ label, icon: Icon }) => (
            <div key={label} className="bg-white/80 border border-bark/10 rounded-xl p-4 flex flex-col items-center gap-2 backdrop-blur-sm shadow-sm">
              <Icon size={20} className="text-bark" />
              <span className="text-xs text-field/60 font-light tracking-wide">{label}</span>
            </div>
          ))}
        </div>
      </div>
